document.addEventListener("DOMContentLoaded", function () {
  ...
  function cambiarIdiomaIndex(idiomaSelect) {
    ...
    // titulo__proyecto.innerHTML = `${idioma.titulo}`;
    // descripcion__proyecto.innerHTML = `${idioma.desc}`;
    //dondencontrar.innerHTML = `${idioma.donde}`;
    ejemplos.innerHTML = `${idioma.ejemplos}`;
    vermasfotos.innerHTML = `${idioma.vermas}`;
    if (
  ...
  document.getElementById("left__button").addEventListener("click", () => {
    currentIndex = (currentIndex - 1 + totalSlides) % totalSlides;
    showSlide(currentIndex);
  });

  document.getElementById("right__button").addEventListener("click", () => {
  ...
  document.querySelectorAll(".trigger-image").forEach((img, index) => {
    img.addEventListener("click", () => {
      currentIndex = index; // Set currentIndex to the index of the clicked image
      showSlide(currentIndex); // Show the corresponding slide

      // Display the image slider
      document.getElementById("image__shower").style.display = "flex";
    });
  });
});
